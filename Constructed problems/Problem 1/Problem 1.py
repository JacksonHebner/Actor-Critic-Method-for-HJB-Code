...
for j in range(start, start + Num_iterate):
    ...
    if (j%2):
        ...
        for count1 in tqdm(range(Num_epo_cri)):
            ...
            #PDE operator -- note that this leaves in unneeded gradients
    ...
    if not (j%2):
        ...
        for count2 in tqdm(range(Num_epo_act)):
            ...
            ih = (sec_order + first_order + cos).mean()
...
torch.jit.script(qnet).save("qnet_P1.pt")
torch.jit.script(unet).save("unet_p1.pt")

#benchmarking
num_sims = int(10e3) #number of Monte Carlo samples of value function
